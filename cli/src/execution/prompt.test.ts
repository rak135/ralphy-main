...
describe("buildPrompt", () => {
	...
	let ralphyDir: string;

	beforeEach(() => {
		testWorkDir = mkdtempSync(join(tmpdir(), "prompt-test-"));
		ralphyDir = join(testWorkDir, ".ralphy");
...
`,
	...
	describe("Prompt Structure", () => {
		...
		it("should include no-commit instruction when autoCommit is true", () => {
			const result = buildPrompt({
				task: "Test task",
				workDir: testWorkDir,
				autoCommit: true,
			...
			expect(result).toContain("after task state is persisted");
			expect(result).not.toContain("Commit your changes with a descriptive message");
		});

		it("should include no-commit instruction when autoCommit is false", () => {
			const result = buildPrompt({
				task: "Test task",
				workDir: testWorkDir,
				autoCommit: false,
...
describe("buildParallelPrompt", () => {
	...
	describe("Boundaries Section", () => {
		...
		beforeEach(() => {
			testWorkDir = mkdtempSync(join(tmpdir(), "parallel-prompt-test-"));
			ralphyDir = join(testWorkDir, ".ralphy");
			mkdirSync(ralphyDir, { recursive: true });
		});
	...
	describe("Prompt Structure", () => {
		...
		it("should include no-commit instruction when allowCommit is true", () => {
			const result = buildParallelPrompt({
				task: "Test task",
				progressFile: ".ralphy/progress.txt",
				allowCommit: true,
