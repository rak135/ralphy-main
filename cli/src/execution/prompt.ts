...
export function buildPrompt(options: PromptOptions): string {
	...
	const codeChangeRules = [
		...
		"Keep changes grouped logically. Do not run git commit from the agent.",
	...
	step++;
	instructions.push(
		`${step}. Do NOT run git commit; leave changes uncommitted so Ralphy or the user can commit after task state is persisted`,
	);
